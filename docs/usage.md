...
The installer prints the Claude Code and OpenAI Codex commands with the right path at the end of the install. By default it puts the server in `~/.linux-computer-use/app`.
...
The `browser` parameter of `open` accepts `chromium`, the default and the bundled build, plus `chrome`, `firefox`, `webkit` and `msedge`. Brave, Opera and Vivaldi are not values here: reach them with `cdp`, as described below. `browser` and `profile` are both ignored when `cdp` is set.
...
It needs the packages listed in [Requirements in detail](#requirements-in-detail). The browser backend is unaffected if they are missing.
...
Architecture notes and the reasoning behind these choices are in [design.md](design.md).
